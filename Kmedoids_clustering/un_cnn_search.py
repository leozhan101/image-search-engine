...
def search(index):
    ...
    min_distance = distance.cosine(queryFeatures[0], centres[0])
    cluster_label = 0
    ...
    for i in range(0, len(centres)):
        if distance.cosine(queryFeatures[0], centres[i]) < min_distance:
            min_distance = distance.cosine(queryFeatures[0], centres[i])
            cluster_label = i
